...
function isMainSellerAccount() {
  return accountType === ACCOUNT_TYPE_MAIN_SELLER || accountType === 'seller';
}
...
function applyAccountTypeUI() {
  const isMain = isMainSellerAccount();
...
async function addOrder() {
  ...
  const payload = {
    date: todayKey(),
    time: new Date().toLocaleTimeString('en-PH', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Manila' }),
